...
class Cube:
    ...
    def find_overlap(self, other: 'Cube'):
        for _ in self.orientations:
            for p1 in self.points:
                other_points = list(other.points)
                for i in range(len(other_points)):
                    p2 = list(other.points)[i]
                    other.offset(p2 - p1)
                    overlap = self.points & other.points
                    if overlap:
                        print(f'Found overlap of {len(overlap)}')
...
class System:
    ...
    def find_overlapping(self, count=12):
        for s1, s2 in permutations(self.system, 2):
            s1.find_overlap(s2)


scanners = [Cube(x.split('\n')[1:]) for x in RAW.strip().split('\n\n')]
system = System(scanners)
cube = system.system[0]
cube1 = system.system[1]

# for ori in cube.orientations:
#     print(ori)
cube.find_overlap(cube1)
